...
static esp_err_t _uvc_vs_commit_control(uvc_stream_ctrl_t *ctrl_set, uvc_stream_ctrl_t *ctrl_probed)
{
    ...
    /* UVC spec: wIndex = VideoStreaming interface number (not always 1 on composite devices). */
    uint16_t vs_ifnum = s_usb_dev.uvc->vs_ifc->interface;
    if (vs_ifnum == 0) {
        vs_ifnum = 1;
    }

    ESP_LOGD(TAG, "SET_CUR Probe (VS interface %u)", vs_ifnum);
    xSemaphoreTake(s_usb_dev.xfer_mutex_hdl, portMAX_DELAY);
    USB_CTRL_UVC_PROBE_SET_REQ((usb_setup_packet_t *)urb_ctrl->transfer.data_buffer);
    ((usb_setup_packet_t *)urb_ctrl->transfer.data_buffer)->wIndex = vs_ifnum;
    ...
    uint32_t mps_limit = s_usb_dev.uvc->vs_ifc->ep_mps;
    if (mps_limit == 0 || mps_limit > USB_EP_ISOC_IN_MAX_MPS) {
        mps_limit = USB_EP_ISOC_IN_MAX_MPS;
    }
    if (ctrl_probed->dwMaxPayloadTransferSize > mps_limit) {
        ESP_LOGW(TAG, "Clamp dwMaxPayloadTransferSize %" PRIu32 " -> %" PRIu32 " (FS host limit)",
                 ctrl_probed->dwMaxPayloadTransferSize, mps_limit);
        ctrl_probed->dwMaxPayloadTransferSize = mps_limit;
    }
